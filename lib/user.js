        ...
        if(results.rows.length === 0) {
          ...
          if(results.rows[0].id !== user_id) {
            return BPromise.reject({
              ...
              code: code.PROVIDER_ALREADY_IN_USE,
              status: 409
            });
          }
        }
      ...
      .then(function(theUser) {
        ...
        if(user[provider] && (user[provider].profile.id !== profile.id)) {
          return BPromise.reject({
            ...
            code: code.ACCOUNT_ALREADY_LINKED,
            status: 409
          });
        }
      ...
      .then(function(theUser) {
        ...
        if(!provider) {
          return BPromise.reject({
            ...
            code: code.MUST_SPECIFY_PROVIDER_TO_UNLINK,
            status: 400
          });
        }
        ...
        if(provider === 'local') {
          return BPromise.reject({
            ...
            code: code.CANNOT_UNLINK_ONLY_PROVIDER,
            status: 400
          });
        }
    ...
    return changePasswordForm.validate()
        ...
        if(user.local && user.local.salt && user.local.derived_key) {
          ...
          if(!form.currentPassword){
            return BPromise.reject({
              ...
              code: code.MUST_SPECIFY_PASSWORD_TO_CHANGE,
              status: 400
            });
          }
      ...
      .then(function() {
        ...
        return BPromise.reject(err || {
          ...
          code: code.WRONG_CURRENT_PASSWORD,
          status: 400
        });
      })
      .then(function() {
    ...
    if(user_id) {
      ...
      if(!session_id) {
        return BPromise.reject({
          ...
          code: code.USER_ID_OR_SESSION_ID_NOT_SPECIFIED,
          status: 401
        });
      }
