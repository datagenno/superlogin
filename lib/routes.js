  ...
  router.post('/logout',
    function (req, res, next) {
      ...
      user.logoutSession(sessionToken)
        .then(function () {
          res.status(200).json({ok: true, success: 'Logged out', code: code.LOGGED_OUT});
        }, function (err) {
      ...
      user.logoutOthers(req.user.key)
        .then(function () {
          res.status(200).json({
            ...
            code: code.OTHER_SESSIONS_LOGGED_OUT
          });
        }, function (err) {
          console.error('Logout failed');
          return next(err);
  ...
  router.post('/logout-all',
    function (req, res, next) {
      ...
      user.logoutUser(null, sessionToken)
        .then(function () {
          res.status(200).json({success: 'Logged out', code: code.UNAUTHORIZED});
        }, function (err) {
      ...
      user.create(req.body, req)
        .then(function (newUser) {
          if(config.getItem('security.loginOnRegistration')) {
            ...
            res.status(201).json({success: 'User created.', code: code.USER_CREATED});
          }
        }, function (err) {
          return next(err);
        });
  ...
  router.post('/forgot-password', function (req, res, next) {
      user.forgotPassword(req.body.email, req).then(function () {
        res.status(200).json({
          ...
          code: code.PASSWORD_RECOVERY_SENT
        });
      }, function (err) {
        return next(err);
      });
        ...
        .then(function (currentUser) {
          if(config.getItem('security.loginOnPasswordReset')) {
            ...
            res.status(200).json({
              ...
              code: code.PASSWORD_RESET_SUCCESS
            });
          }
        }, function (err) {
          return next(err);
  ...
  router.post('/unlink/:provider',
    ...
    function(req, res, next) {
      ...
      user.unlink(req.user._id, provider)
        .then(function() {
          var provider = util.capitalizeFirstLetter(provider);
          res.status(200).json({
            success: provider + ' unlinked',
            provider: provider,
            code: code.PROVIDER_UNLINKED
          });
        }, function (err) {
          return next(err);
        });
  ...
  router.get('/confirm-email/:token', function (req, res, next) {
    ...
    if (!req.params.token) {
      var err = {
        ...
        code: code.EMAIL_VERIFICATION_TOKEN_REQUIRED
      };
      if(redirectURL) {
        ...
        .then(function(err) {
          if(!err) {
            ...
            res.status(409).json({
              ...
              code: code.USERNAME_ALREADY_IN_USE
            });
          }
        }, function(err) {
          return next(err);
